...
module bmp_parser #(
    ...
    // done: 到达像素起始偏移; 新文件开始(byte_idx==0)自动清零以便复用
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)                            done <= 1'b0;
        else if (din_valid && byte_idx == 16'd0) done <= 1'b0;   // 新文件
        else if (din_valid && byte_idx == data_offset) done <= 1'b1;
        else if (!din_valid)                   done <= done;     // 保持
    end

    // 魔数校验
    always @(*) bmp_ok = (magic0 == 8'h42) && (magic1 == 8'h4D);   // 'B','M'
