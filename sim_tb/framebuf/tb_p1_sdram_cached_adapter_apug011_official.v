...
// ================================================================
// P1-05A cached-adapter official protected-core compatibility probe
//
// Chain:
//   P1 abstract memory request
...
//   SDRAM hardware/frequency/phase.  SFT_OFFSET_PS remains parameterized here;
//   the canonical compatibility run uses 180deg.
//
// Vendor RTL/model files are never modified by this TB.
// ================================================================

module tb_p1_sdram_cached_adapter_apug011_official #(
    ...
    p1_sdram_cached_adapter dut_adapter (
        .clk                         (clk),
        .rst_n                       (rst_n),
    ...
    initial begin
        ...
        $display("OFFICIAL_READ_COUNT: accepted=%0d hits=%0d misses=%0d app_reads=%0d",
                 read_accept_count_debug, read_cache_hit_count_debug,
                 read_cache_miss_count_debug, app_read_word_count_debug);
        ...
        if (errors == 0)
            $display("PASS: cached adapter official APUG011 compatibility passed (checks=%0d, half=%0dps, sft=%0dps)", checks, HALF_CLK_PS, SFT_OFFSET_PS);
        else
