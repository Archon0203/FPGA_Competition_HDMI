...
## 角色
- 张宗（队员A / 所有者）：审核并合并 PR，维护 main 与文档。
- 曾雨婷（队长 / Collaborator）：统筹、开发、经授权可代审；开发走上游 feature 分支 + PR。
- 杨文轩（队员B / Collaborator）：开发、开 PR；可走上游分支，也可 fork 隔离。

## 提交流程（一句话）
clone 原仓库 -> 建 feature 分支 -> 改 -> ModelSim 自检 PASS -> commit -> push -> PR 到 main -> 等 Approve -> Squash and merge
...
git config user.name "张宗"; git config user.email "你的邮箱@users.noreply.github.com"
