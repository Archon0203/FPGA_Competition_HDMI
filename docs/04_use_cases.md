...
UI 逻辑按 **1280×720 性能目标画布**预留，但当前 board-safe bring-up 先使用与官方样例一致的 640×480；后续 720p timing 闭合后再统一定标。顶部状态栏和底部滚动标语的具体高度在 P2 按 720p 字体资源重新定标；应急模式时 UI-L3 覆盖普通信息。
...
1. **上电基线**：先以 640×480 board-safe HDMI 稳定显示默认公告/测试画面；720p 在 timing 闭合后升级。
...
| 转场 | 图片淡入/擦拭 |
| 缩放 | 不同源分辨率适配当前输出模式，并为 1280×720 性能目标预留 |
...
- “640×480 是当前 board-safe bring-up 基线，1280×720 是性能目标”；
...
- “720p 只有在 TD timing + 真板稳定证据齐全后才可宣称通过”；
- “1080p 支持”；
- “MP4/H.264 实时解码”；
- “整条 SD→显示链微秒级”。
...
P1-04C 已完成 HDMI_B 实际输出验证，可作为后续图片播放、OSD 和短视频展示的显示底座。
